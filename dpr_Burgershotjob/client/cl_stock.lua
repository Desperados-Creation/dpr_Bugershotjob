...
-- Menu --
local open = false
local MenuStockBurgershot = RageUI.CreateMenu("Stock", "INTERACTION")
MenuStockBurgershot.Display.Header = true
MenuStockBurgershot.Closed = function()
    open = false
end

function OpenMenuStockBurgershot()
    if open then
        open = false
        RageUI.Visible(MenuStockBurgershot, false)
        return
    else
        open = true
        RageUI.Visible(MenuStockBurgershot, true)
        CreateThread(function()
            while open do 
                RageUI.IsVisible(MenuStockBurgershot, function()
                    RageUI.Separator("↓    ~o~Stock    ~s~↓")
...
Citizen.CreateThread(function()
    while true do
        ...
        if ESX.PlayerData.job and ESX.PlayerData.job.name == 'burgershot' then
			for k in pairs(Config.Position.Stock) do
                local plyCoords = GetEntityCoords(GetPlayerPed(-1), false)
                local pos = Config.Position.Stock
                ...
                if dist <= 1.0 then
                    wait = 0
                    Visual.Subtitle(Config.TextStock, 1)
                    if IsControlJustPressed(1,51) then
                        OpenMenuStockBurgershot()
                    end
                end
            end
		end
